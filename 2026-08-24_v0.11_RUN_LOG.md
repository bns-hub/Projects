...
## Session 31b (v0.11) — 2026-08-24 — review pass over the session's own work

Benson: *"do a pass on what has been done"*, and separately *"your standing rules and run log has no
version.... v0.11"*. Both handled here. Nothing in the HTML changed for this pass — it is a
correctness audit of the session's output plus a delivery-format fix.

### Filenames now carry the version (real miss on my part)
He had already asked once that the docs "have vX.X as a label and match the version of the html
output", and I had bumped only the in-file header (`**Doc version: v0.11**`) while the delivered
files were still bare `STANDING_RULES.md` / `RUN_LOG.md`. On a file card that reads as unversioned,
which is exactly what he flagged the second time. All three deliverables now share one prefix:
`2026-08-24_v0.11_STANDING_RULES.md`, `2026-08-24_v0.11_RUN_LOG.md`,
`2026-08-24_v0.11_RE1999TeamBuilder.html`. Convention recorded in §2 so it isn't lost next run.

### Audit results — the session's structural claims hold
Checked against the SHIPPED file, not from memory:
- **Cumulative declaration diff across the whole session, v0.1 → v0.11: 176 → 192, with exactly ONE
  removal** — `CONDUIT_CARD_EFFECTS`, the documented v0.7 rename to `CONDUIT_KIT`. Nothing else was
  lost across eleven versions of edits, which is the property §8's lesson exists to protect.
- **All 17 declarations added this session are wired** (2–13 references each). Zero dead data. The
  one that was dead when written (`sbIsConduitModelled`, v0.7) had already been caught and removed
  before shipping.
- Live characters **129**, `ROLE_OVERRIDE` **130 entries, 0 live missing a Role, 0 out-of-taxonomy
  values**, Cornerstone `upcoming:true`, Twins deck **6** card types modelled, Coppélia **3** — every
  figure the docs assert is true of the build.

### One real error found in my own docs, and it is the §5 failure mode
**`PORTRAY_SIMULATED` covers 7 levels across 5 characters, not 6.** The docs have said "6 levels
across 5 characters" in eight separate places since v0.2. The true content is Rhiannon P1,
Flutterpage P1, Lorentz Butterfly P1, **Cheng Heguang P1+P2**, **Liang Yue P1+P2** = **7**. The "6"
was wrong from the moment it was written — it appears to have been copied from the count of
`PORTRAY_MECHANIC` tagged entries rather than counted from `PORTRAY_SIMULATED` itself.

This is precisely the failure §5 warns about: *"Do not maintain 'X of 132 rebuilt' as prose... The
moment a number like that is written down here, it starts drifting."* It drifted immediately, then
was carried forward verbatim through nine versions because each entry copied the previous one's
open-items block.

**Historical entries are left as written** (append-only; they record what was believed at the time,
and rewriting them is the thing §0/§HTML-version guidance explicitly forbids). The correct figure is
**7**, and any future entry must state 7 — or better, per §5, compute it rather than copy it:
`Object.values(PORTRAY_SIMULATED).reduce((a,o)=>a+Object.keys(o).length,0)`.

### Open items (unchanged from Session 31, with the count corrected)
1. **Which 6 cards the battle-start Integration screen offers**, and on what basis — unsourced.
2. **Energy-deck copy counts and per-round draw** — now confirmed a REAL mechanic (Session 31), size
   still unknown. The "10 total Energy Card types vs 6 modelled" gap is the concrete lead.
3. **The Twins' "AP +1"** — still needs Benson to settle (§24.4); displayed, not silently changed.
4. **Conduit math untested against a real match.**
5. **`PORTRAY_SIMULATED` covers 7 levels across 5 characters** (corrected above).
6. Per-card AP costs unsourced (default 1); search-sourced numbers pending verbatim re-verification
   (§19.0); Cornerstone kit data retained while `upcoming:true`.

Unchanged: 32 characters produce no stat effects; 51 effect instances dropped; effect layer is a bulk
pass; Portray backlog spot-checking (16 of 126); `BUFF_STACK_MODE` unverified; damage model untested;
`ULT_HOLD_OVERRIDE` unwired; `AP_SURPLUS_OVERRIDE` unused; Corvus monotonic counter; Ezio
Synchronization underestimate; Kassandra card-injection thresholds; Cheng Heguang's ≥10 `[Feathered
Blades]`; Beryl's Emanation crystal; Tuning card interaction unmodeled; Matilda / Lady by the Lake
rarity; The Twins' dual element; An-an Lee's portrait; Everecho has no `PORTRAY_DB` data.

→ No HTML change this pass; `2026-08-24_v0.11_RE1999TeamBuilder.html` stands as delivered.
