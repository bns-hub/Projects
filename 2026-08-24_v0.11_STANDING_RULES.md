...
- **Filenames carry the version, same convention as the HTML (Benson, 2026-08-24: "run log and standing rule should have vX.X as a label and should match the version of the html output"; then "your standing rules and run log has no version.... v0.11" when only the in-file header carried it).** All three delivered files share one prefix: `<date>_v<ver>_STANDING_RULES.md`, `<date>_v<ver>_RUN_LOG.md`, `<date>_v<ver>_RE1999TeamBuilder.html`. The version must be visible on the FILE, not only in the header inside it — a file card showing a bare `STANDING_RULES.md` is the thing he was objecting to. Bump all three together, always.  
- This rules file lives in the working folder — its own Drive file ID isn't self-tracked here (it changes on every re-upload since Drive has no in-place text edit); locate it by title search each run (search `STANDING_RULES`, which still matches the versioned filename).  
- Companion RUN_LOG — same working folder, same "locate by title search" caveat (search `RUN_LOG`).  
